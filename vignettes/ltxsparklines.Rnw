...
options(ltxsparklines.output='inlineSweave') # Comment out if using knitr
@ 
...
plot_matrix <- "mat <- matrix(c(1, 2, 3, 4, 5, 17, 10, 12, 11, 10), 
                ...
                startdotcolor='blue')"
plot_ts <- "sparkline(window(Nile, 1880, 1890), 
                      rectangle=quantile(Nile, c(0.25, 0.75)))"
...
dots <- 'sparkline(c(3,5,4,12,9), xdots=2, ydots=5, 
                   dotcolor="red")'
@ 
\fi
...
<<results=tex>>=
# This chunk was set with <<results=tex>>=
# Change to <<results='asis'>>= if using knitr
cat(sparkline(c(3,5,4,12,9), xdots=2, ydots=5, 
                   dotcolor="red", output='knitr'))
@ 
